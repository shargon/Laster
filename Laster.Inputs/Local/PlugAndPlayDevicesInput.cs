    ...
    public class PlugAndPlayDevicesInput : IDataInput
    {
        ...
        public class PnpDevice
        {
            ...
            void ChangeEnable(bool enable)
            {
                ...
                try
                {
                    searcher = new ManagementObjectSearcher("root\\CIMV2", "SELECT * FROM Win32_PnPEntity WHERE DeviceID='" +
                        DeviceID.Replace("\\", "\\\\") + "'");
                    ...
                    foreach (ManagementObject queryObj in searcher.Get())
                        try
                        {
                            ...
                            //if (!enable)
                            //{
                            //    object oreqReboot = queryObj.InvokeMethod("Disable", new object[] { reqReboot });
                            //}
                            //else
                            //{
                            //    object oreqReboot = queryObj.InvokeMethod("Enable", new object[] { reqReboot });
                            //}
                        }
                        catch { }
